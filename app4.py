...
with st.sidebar:
    ...
    start_date = st.date_input("Pick Your Start Date",
                             min_value=datetime(2011, 1, 1),  # Adjusted to ensure enough data
                             max_value=datetime(2020, 12, 31),
                             value=datetime(2019, 1, 1))
    end_date = st.date_input("Pick Your End Date",
...
with st.container():
    ...
    if st.session_state.predictions_generated and start_date <= end_date:
        # Ensure enough historical data before start_date
        ...
        if start_date < min_date + timedelta(days=5):
            ...
            if not filtered_data.empty:
                ...
                if X is None:
                    ...
                    # Create Candlestick and Predicted Line Chart
                    ...
                    st.markdown(f"""
                        - **Market Trend Alert**: An {trend} trend is forecasted, signaling a {trend} market movement — consider {['aggressive investment', 'cautious divestment'][trend == 'downward']} strategies.
                        - **Price Snapshot**:
                         - **Start**: ${start_price:.2f} on {pred_df['Date'].iloc[0].strftime('%Y-%m-%d')}
                         - **Ending**: ${end_price:.2f} on {pred_df['Date'].iloc[-1].strftime('%Y-%m-%d')}
                        - **Profit Potential**: Expect a ${price_increase:.2f} increase ({price_increase_pct:.2f}% change) — optimize stock allocation accordingly.
                        - **Volatility Check**: With a standard deviation of {volatility:.2f}, {'high volatility' if volatility > 5 else 'stable conditions'} suggest {['hedging risks', 'confident expansion'][volatility <= 5]}.
